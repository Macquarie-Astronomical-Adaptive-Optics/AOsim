...
def _compute_basis_gram_matrix(R, *, chunk_modes: int = 64, dtype=np.float32) -> np.ndarray | None:
    """Compute the modal-basis Gram matrix used by ModalTruthProjector.

    The telemetry truth projector currently stores diagonal-projection modal
    coefficients

        c_i = <B_i, phi> / <B_i, B_i>,

    where the inner product is over the pupil.  For Gram-aware post-processing
    we need

        G_ij = <B_i, B_j>.

    This is computed in chunks from ``R._basis_flat`` without saving the full
    basis cube to disk.  The result is small for the current ULTIMATE setup
    (1004 x 1004 float32 is about 4 MB), but the calculation can take a few
    seconds because it touches all basis pixels.
    """
    if not hasattr(R, "_basis_flat") or R._basis_flat is None:
        return None
    try:
        ...
        n_basis = int(B.shape[0])
        if n_basis <= 0:
            return None
        if hasattr(R, "pupil"):
            pupil_flat = cp.asarray(R.pupil, dtype=cp.float32).ravel()
        else:
            pupil_flat = cp.ones((int(B.shape[1]),), dtype=cp.float32)
        chunk = max(1, int(chunk_modes))
        G = cp.empty((n_basis, n_basis), dtype=cp.float32)
        for i0 in range(0, n_basis, chunk):
            i1 = min(n_basis, i0 + chunk)
            Bi = B[i0:i1].astype(cp.float32, copy=False) * pupil_flat[None, :]
            for j0 in range(i0, n_basis, chunk):
                j1 = min(n_basis, j0 + chunk)
                Bj = B[j0:j1].astype(cp.float32, copy=False) * pupil_flat[None, :]
                block = Bi @ Bj.T
                G[i0:i1, j0:j1] = block
                if j0 != i0:
                    G[j0:j1, i0:i1] = block.T
        G_np = cp.asnumpy(G).astype(dtype, copy=False)
        return 0.5 * (G_np + G_np.T)
    except Exception:
        return None
...
def _save_control_bundle(
    *,
    ...
    if include_gram:
        G_basis = _compute_basis_gram_matrix(R, chunk_modes=gram_chunk_modes, dtype=np.float32)
        if G_basis is not None:
            arrays["basis_gram"] = G_basis
            arrays["basis_gram_diag"] = np.diag(G_basis).astype(np.float32, copy=False)
    ...
    summary = {
        ...
        "notes": {
            ...
            "basis_gram": "Modal basis Gram matrix G_ij=<B_i,B_j> over the pupil, used to convert diagonal truth projections into Gram-aware least-squares coefficients.",
        },
        "reconstructor": {
...
class AOBatchRunner:
    ...
    def __init__(
        ...
        logger.debug("wfs_is_lgs=%s", self._wfs_is_lgs)
        ...
        self._tt_recon = TipTiltReconstructor_CuPy(
            ...
            sensor_method=str(self.wfs_method),
            params=self.params,
        )
    ...
    def run(
        ...
        control_bundle_include_gram: bool = False,
        control_bundle_gram_chunk_modes: int = 64,
    ) -> Dict[str, object]:
        ...
        D = float(self.params.get("telescope_diameter"))
        # Prefer the science sensor's wavelength, fall back to params.
        lam_sci = float(getattr(self.sensors[0], "wavelength", self.params.get("science_lambda")))
        ...
        if bool(save_control_bundle):
            control_bundle_result = _save_control_bundle(
                ...
                include_gram=bool(control_bundle_include_gram),
                gram_chunk_modes=int(control_bundle_gram_chunk_modes),
            )
            ...
            for g in self._wfs_groups:
                ...
                for jj, wi in enumerate(inds):
                    ...
                    if do_tt_update and (not self._wfs_is_lgs[wi]):
                        ...
                        mu = self._tt_recon._weighted_mean_yx(sl_err, wi)
                        mu = mu * (wl / cp.float32(500e-9))
                        # Convert centroid-bin mean -> phase-ramp slope (rad/pixel @500nm-ref)
                        try:
                            if wi < len(self._tt_recon.k500_yx):
                                k = self._tt_recon.k500_yx[wi]
                                mu = mu / cp.maximum(k, cp.float32(1e-20))
                        except Exception:
                            pass
                        tt_ctrl_acc += mu
                        tt_ctrl_n += 1
