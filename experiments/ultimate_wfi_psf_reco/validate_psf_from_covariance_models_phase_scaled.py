...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--phase-scale-multiplier",
        type=float,
        default=1.0,
        help=(
            "Extra multiplier applied after --basis-units conversion. "
            "Use wfs_lambda/science_lambda when basis maps are phase at the WFS/reference "
            "wavelength but the PSF is synthesized at the science wavelength."
        ),
    )
...
def main() -> None:
    ...
    phase_scale_base = phase_scale_factor(args.basis_units, args.wavelength_m)
    phase_scale = phase_scale_base * float(args.phase_scale_multiplier)
    ...
    summary = {
        ...
        "phase_scale_base": float(phase_scale_base),
        "phase_scale_multiplier": float(args.phase_scale_multiplier),
    ...
    print(
        f"Basis units: {args.basis_units}; "
        f"base phase scale={phase_scale_base:.6g}; "
        f"multiplier={args.phase_scale_multiplier:.6g}; "
        f"total phase scale={phase_scale:.6g}"
    )
